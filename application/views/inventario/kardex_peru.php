<div class="md:ml-64 min-h-screen bg-slate-50 pt-20">
    ...
    <section class="px-4 md:px-8 py-5 md:py-6 border-b bg-white shadow-sm">
        <h1 class="text-xl md:text-3xl font-bold text-slate-800 flex items-center gap-3">
            <i class="fas fa-history text-slate-600"></i> Kardex de Inventario - Perú
        </h1>
    </section>
    ...
    <section class="p-4 md:p-8 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow p-4">
            ...
            <form action="<?= base_url('inventario/kardex_peru') ?>" method="GET" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-10 gap-4">
                    ...
                    <div class="md:col-span-4">
                        ...
                        <select name="producto_id" class="form-control select2 w-full border border-slate-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
                            <option value="">-- Todos los productos --</option>
                            <?php foreach($productos as $p): ?>
                                <option value="<?= $p->id  ?>" <?= $this->input->get('producto_id') == $p->id ? 'selected' : '' ?>>
                                    <?= $p->nombre ?> <?= $p->color ?> <?= $p->talla ?> (<?= $p->codigo ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2 flex gap-2">
                        ...
                        <a href="<?= base_url('inventario/kardex_peru') ?>" class="w-full bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 rounded-lg flex items-center justify-center gap-2">
    ...
    $j(document).ready(function() {
        ...
        if ($j.isFunction($j.fn.DataTable)) {
            $j('#tabla-kardex').DataTable({
                ...
                "searching": true,       // Buscador interno rápido
